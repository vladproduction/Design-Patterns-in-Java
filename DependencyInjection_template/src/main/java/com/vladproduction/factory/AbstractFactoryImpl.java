...
public class AbstractFactoryImpl implements AbstractFactory {
    ...
    public <T> T create(Class<T> clazz) {
        ...
        //file=variant#2
        if (FileIntegrationService.class.isAssignableFrom(clazz)) { //singleton??? experimental, could not work
                ...
                .newProxyInstance(clazz.getClassLoader(),new Class[]{clazz}, new InvocationHandler() {
                    ...
                    private Object behaviourOfGet(Method method,Object[] args) {
                        ...
                        if (selectorVariant.length > 0 && args != null && args.length >= selectorVariant.length) {
                            return getSqlQueryLogic(args, tableBDName, selectorVariant, tableNameChooseParams);
                    ...
                    //variant#1-getSql
                    private static String getSql(Object[] args, String tableBDName,String[] selectorVariant,
                                                 String[] tableNameChooseParams) {

                        StringBuffer sqlQueryBuilder = new StringBuffer(
                                String.format("SELECT * FROM %s WHERE ", tableBDName.toLowerCase()));
                    ...
                    //variant#2-getSqlQueryLogic
                    private static String getSqlQueryLogic(Object[] args, String tableBDName, String[] selectorVariant,
                                                           String[] tableNameChooseParams) {
                        ...
                        for (int i = 0; i < selectorVariant.length; i++) {
                            ...
                            if (i < selectorVariant.length - 1) { //todo
                                boolean b = tableNameChooseParams[1].contains("And");
                                if (b) {sqlQueryBuilder.append(" AND ");}
                                else if (!b){sqlQueryBuilder.append(" OR "); }
